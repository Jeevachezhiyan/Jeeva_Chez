...
class ShippingInformationManagementPlugin
{
    ...
    public function beforeSaveAddressInformation(
        ...
        if($this->_helper->getDisplayAt() == self::DELIVERY_FORM_DISPLAY_AT_SHIPPING_ADDRESS || $this->_helper->getDisplayAt() == self::DELIVERY_FORM_DISPLAY_AT_SHIPPING_METHOD){
	  ...
	  $orderComments = $extAttributes->getOrderComments();
          $traceNumber = $extAttributes->getDeliveryTimeSlot();
	  ...
	  if($shippingMethod == "flatrate" && strlen($shippingArrivalComments) == 0) 
	         throw new \Magento\Framework\Exception\LocalizedException(__("Please fill the reason for Rush Order."));

          $quote->setShippingArrivalDate($shippingArrivalDate)
                ->setShippingArrivalTimeslot($deliveryTimeSlot)
                ->setShippingArrivalComments($shippingArrivalComments)
